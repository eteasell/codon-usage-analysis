'''
This file contains calculation functions, such as for RSCU or elongation times.
'''

import numpy as np
from Bio.SeqRecord import SeqRecord
from lib.aminoacids import AA_TO_CODONS_MULTI_CODON_FAMILIES, CODON_TO_AA_MULTI_CODON_FAMILIES
from lib.genetic_data_helpers import *
from lib.data_access_helpers import load_elongation_rates
...
def calculate_elongation_time(codons: list[str]) -> float:
    '''
    codons: a list of codons in the sequence
    
    This function calculates total time of elongation for the given sequence.
    ASSUMES THAT THE SEQUENCE VALIDITY HAS BEEN CHECKED BY ensure_valid_orf.
    
    time = sum over 1/r_i for every codon i in window, 
    where r_i is the experimentally derived median elongation rate for each codon, in codons/second
    '''
    df_elongation = load_elongation_rates()
    
    rates = []
    
    for codon in codons:
        rate = df_elongation.loc[codon]['median_elongation_rate']
        rates.append(rate)
        
    rates_arr = np.array(rates)
    time_arr = np.reciprocal(rates_arr)
